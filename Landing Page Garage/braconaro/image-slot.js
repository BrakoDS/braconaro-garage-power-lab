...
(() => {
  ...
  // Standalone (published) fallback: outside the Claude Design runtime there is
  // no window.omelette host bridge, so persist dropped images in localStorage
  // instead. Keeps drag-drop working and surviving reloads on a real site.
  const HAS_LS = (() => { try { return !!window.localStorage; } catch (e) { return false; } })();
  function readSidecar() {
    if (HAS_LS) {
      try {
        const raw = window.localStorage.getItem(STATE_FILE);
        if (raw) return Promise.resolve(JSON.parse(raw));
      } catch (e) {}
    }
    return fetch(STATE_FILE).then((r) => (r.ok ? r.json() : null)).catch(() => null);
  }
  ...
  function save() {
    ...
    let p;
    if (w) {
      p = Promise.resolve(w(STATE_FILE, JSON.stringify(slots)));
    } else if (HAS_LS) {
      try { window.localStorage.setItem(STATE_FILE, JSON.stringify(slots)); } catch (e) {}
      p = Promise.resolve();
    } else {
      saving = false; return;
    }
