...
export function instrucoes(textoDigitado: string): string {
  ...
  if (texto) {
    base.push(
      ...
      '',
      'O TEXTO DIGITADO CARREGA AS MESMAS CORES DA LOUSA, marcadas assim:',
      '  [[vermelho]]…[[/vermelho]] → intensidade, RIR, descanso, observação de carga ("observacao")',
      '  [[azul]]…[[/azul]]         → séries, repetições, estações ("series" e "reps")',
      '  texto sem marca            → preto: títulos, blocos e nomes de exercício',
      'As marcas são do sistema, NÃO fazem parte do treino: nunca as copie para dentro de',
      'um nome, de uma repetição ou de uma observação. Colchete simples que o coach',
      'escreveu ("[3 rounds]") é conteúdo e deve ser lido normalmente.',
      '',
      '--- TEXTO DIGITADO ---',
      texto.slice(0, 4000),
      '--- FIM ---',
