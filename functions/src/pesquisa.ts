 ...
 * real:
 *   - `PADROES`         ← `compartilhado/config/padroes.js` (`PADROES`, 6 valores;
...
/** `PADROES` de `compartilhado/config/padroes.js` — chaves, não rótulos. */
const PADROES = [
...
/** `TAGS` de `coach/academia/db.js`. */
const TAGS = ['MUSCULAÇÃO', 'HYROX', 'HIIT', 'CROSS', 'GAP', 'MOBILIDADE'] as const;

/** `Exercicio.nivel` de `compartilhado/dados/exercicios.js`. */
const NIVEIS = ['iniciante', 'intermediario', 'avancado'] as const;

/** Nível padrão quando a IA manda algo fora do vocabulário — mesmo fallback de `coach/academia/db.js` (`x.nivel || 'intermediario'`). */
const NIVEL_PADRAO = 'intermediario';

/** `tempoMedioSeg` só é aceito nesta faixa; fora dela cai no padrão do contexto. */
const TEMPO_MIN = 5;
const TEMPO_MAX = 600;
/** Padrão de exercício "normal" — mesmo valor usado em boa parte de `compartilhado/dados/exercicios.js`. */
...
export function instrucoes(contexto: Contexto, equipamentos: Equip[]): string {
  ...
  return [
    ...
    '- Em "equipamentoIds" marque SÓ o equipamento em que o exercício é DE FATO',
    '  executado. Se nada na lista serve de verdade, NÃO force um encaixe só',
    '  porque o nome parece próximo: marque o item de peso corporal, se houver, e',
    '  diga o que falta.',
    '- O que o exercício normalmente pede e este box NÃO tem vai em texto livre (o',
    '  nome do aparelho, não um id) em "equipamentoFaltante". Deixar esse campo',
    '  vazio quando falta algo é PIOR que errar o exercício: o coach cadastra',
    '  achando que dá para fazer na academia dele.',
    '  Exemplo do erro a evitar: um muscle up pede barra fixa ou argolas; se o box',
    '  só tem puxadores de polia, eles NÃO servem — o certo é peso corporal em',
    '  "equipamentoIds" e "barra fixa ou argolas" em "equipamentoFaltante".',
    ...
    '  Escreva em português do Brasil correto e revise a conjugação: este texto vai',
    '  para a ficha que o coach lê e copia na lousa, sem passar por revisor.',
    '- "fontes": URLs (começando com http) de onde você tirou a informação;',
    '  lista vazia se não tiver fonte confiável.',
  ].join('\n');
}
